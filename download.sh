
mkdir data
wget -P ./data "http://index.commoncrawl.org/CC-MAIN-2018-39-index?url=*.mc&output=json&page=0"
wget -P ./data "http://index.commoncrawl.org/CC-MAIN-2018-39-index?url=*.mc&output=json&page=1"
wget -P ./data "http://index.commoncrawl.org/CC-MAIN-2018-39-index?url=*.mc&output=json&page=2"
wget -P ./data "http://index.commoncrawl.org/CC-MAIN-2018-39-index?url=*.mc&output=json&page=3"
wget -P ./data "http://index.commoncrawl.org/CC-MAIN-2018-39-index?url=*.mc&output=json&page=4"
wget -P ./data "http://index.commoncrawl.org/CC-MAIN-2018-39-index?url=*.mc&output=json&page=5"
